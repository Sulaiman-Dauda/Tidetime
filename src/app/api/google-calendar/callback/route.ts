...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL("/dashboard/settings?google_error=" + encodeURIComponent(error), req.url),
    );
  }

  if (!code || !state) {
    return NextResponse.redirect(
      new URL("/dashboard/settings?google_error=missing_code_or_state", req.url),
    );
  }
  ...
  if (!userId) {
    return NextResponse.redirect(
      new URL("/dashboard/settings?google_error=invalid_state", req.url),
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      new URL(
        "/dashboard/settings?google_error=" +
